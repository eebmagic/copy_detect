...
if len(sys.argv[:]) > 1 and sys.argv[1] and sys.argv[1] not in accetable_cli_options:
	print(f"###ERROR: No such option: '{sys.argv[1]}'")
	quit()
...
### If Directory_PATH is in roster ###
	...
	### If complete match ###
	if set(currentImages) == oldImages and sigFileFound:
		exactMatch = True
	...
	elif set(currentImages) != oldImages and sigFileFound:
...
## Make every possible combination of pictures in the whole directory ##
combos = list(itertools.combinations(currentImage_PATHS, 2))

'''
print(combos)
quit("Here are all the combos: ^")
'''
...
for match in matches:
	print(match[0][len(Directory_PATH):], match[1][len(Directory_PATH):])
